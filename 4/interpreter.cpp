...
void Interpreter::analyze() {
    ...
    for (const auto &token: tokens) {
        ...
        cout << divider << endl << "Errors: " << endl << divider << endl << errors.str();
        cout << divider << endl << "TACs: " << endl << divider << endl;
        if (errors.str().empty()) {
            for (auto quadruple: analyzer.getQuadruples()) {
                cout << quadruple.toString() << endl;
            }
        }
    }
}
...
void Interpreter::compile() {
    *output << mips.compile(analyzer.getQuadruples());
}
...
void Interpreter::switchOutputStream(ostream *os) {
    output = os;
}
